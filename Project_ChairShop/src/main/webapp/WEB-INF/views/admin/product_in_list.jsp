...
<script type="text/javascript">
	...
	function delete_in(pm_idx){
		if(confirm("정말 입고를 취소하시겠습니까?")==false) return;
		
		location.href="delete_in.do?pm_idx=" + pm_idx;
	}
	 ...
	 window.onload=function(){
		 ...
		 function show_message(){
			 ...
			 if("${ param.error eq 'in_not' }"=='true')
			 {
				 alert('상품 리스트에 없는 상품입니다\n상품 등록 후에 입고 처리할 수 있습니다.');
			 }
		 }
	 };
			...
			<h1>입고관리</h1>
				...
				<div class="insert_manage">
					<form>
						...
						수량 <input name="p_cnt">
						<input class="insert_btn" type="button" value="입고" onclick="insert_in(this.form);">
					</form>
				</div>
				<div class="list_info">
					<ul>
				...
				<div class="list">
					<c:forEach var="vo" items="${ map.in_list }">
						<ul>
							...
							<li><input class="delete_btn" type="button" value="입고취소" onclick="delete_in('${ vo.pm_idx }');"></li>
						</ul>
					</c:forEach>
				</div>						
			</div>
